...
const AddReceipe = () => {
    ...
    return (
        <section className="flex h-screen w-screen bg-white bg-opacity-50 justify-center items-center absolute top-0 right-0">
            <div className="w-10/12 h-4/5 bg-yellow-200 rounded-2xl py-4">
                <Link className="absolute top-4 left-4" to="/receipes">
                ...
                <form className="flex flex-col h-full overflow-auto">
                    <div className="flex flex-col h-full overflow-auto">
                        ...
                        <div className="px-4 mb-3 h-1/7">
                            ...
                            <input type="number" name="duration" value={duration} placeholder="duration" required onChange={(e: React.FormEvent<HTMLInputElement>) => setDuration(e.currentTarget.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline border-blue-300 " />
                        </div>
                        <div className="flex overflow-auto px-2">
                            {ingredients.map(ingredient => <p className="mb-3 whitespace-nowrap px-4 mx-1 py-1mx-16 border-2 border-gray-200 rounded-2xl bg-yellow-100 ">{ingredient.name} - {ingredient.quantity}</p>)}
                        </div>
                        ...
                        <div onClick={addIngredient} className="h-1/7 mx-16 rounded-2xl bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-4 rounded focus:outline-none focus:shadow-outline">
                            Add ingredient
                        </div>
                    </div>
